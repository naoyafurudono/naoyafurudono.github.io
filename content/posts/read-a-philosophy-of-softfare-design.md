...
例外というか、異常系をなくすような設計にしようと主張している。

10.2 の「例外を投げるのは簡単だけど、ハンドリングするのは難しい」という観察が箴言だと思う。
複雑さをモジュールの中に押さえ込んでシステム全体をシンプルにするために（これはこの本全体のメッセージ）、異常系が存在しないように（見えるように）定義するのが良い（この章で着目する具体的なポイント）と理解した。

この原則から以下のようなベスプラが説明できる

- APIを冪等にする
- 勝手にリトライとかして細かい異常系を見せなくする
  - 世の便利ソフトウェアはこういうことをする。なのでなんか上手く動いていなかったらAPIのレスポンスだけじゃなくて、そのソフトウェアのログを見ると詳細を見られて良い。
- まとめてハンドリングしちゃう
  - 例外を出す側だけじゃなくて、ハンドリングする側も工夫できる
  - 例えばWebサーバでよくわからないエラーは5xxを返すようにハンドリングしちゃうとか
  - 404も実はいい感じに出せそう
- 例外を投げないでパニックする

だんだん馴染んできた。多くの主張が当たり前じゃんと思えるようになってきたように感じる。

# Chapter 11. Design it Twice

Just Do It おじさんみたいな回だった。

初手でいいデザインできることって普通はないのでいいから何回かデザインしましょう、という話。
何回かデザインした上で、それぞれの案同士で pros, cons を考えて比較する、使う側にとって使いやすいインターフェースになっていることを担保する。
そのために、インターフェースがシンプルか、汎用的か、効率の良い実装を可能にするか、といった観点がある。

どの案も微妙なことはあるはずで、そのときは出した案をもとにいいとこどりをした最強のデザインを改めて出せばいい。

時間がかかるとか、自分は十分に賢いとかはそんなことないので気にしにないでいいから複数のデザインをしてそれらを比較して良いやつを作ろう、と述べている。スキルも上がるし。

ついつい初手でまあこれだろうという設計をしちゃうと、二つ目を考えたくなくなる。それで良いわけがないので、何個か案を作ろう。

# Chapter 12. Why Write Comments? The Four Excuses

コメントを書こうという話で、この章ではコメントを書くモチベーションと、コメントを避ける言い訳に対する否定を説明している。

この段落は僕の持論なのですが、コメントはプログラミング言語のシンタックスの用語だと思ってる。
その目的とか意味は単にコメントと述べただけでは定まらない。
例えばダイレクティブはコメントとして書かれるコンパイラに対する命令である一方で、TODOコメントは開発する上でのメモに過ぎない。
このチャプターでは著者の考えるコメントを用いて何を表現するべきか、それによってどのようにソフトウェアの複雑さを下げられるかを議論するとも捉えられそう。

持論終わり、言い訳とそれに対する説明は以下のような感じだと理解した。

- いいコードは自身が説明的である: 複雑さを隠せていないということになるので良くない
- コメントを書く時間がない: どうせ10%くらいタイピングする時間が増えるくらいだろうし、長い目で見るとメンテナンスしやすくなるのだからペイするはず。それに設計のツールとしても使えるのだから書けばいい
- コメントは古くなる: 更新すればいいし、そんなに大変でもない。コードよりはコメントの方が更新しないといけない頻度は少ないし。コードの近くにドキュメントを置く、みたいな努力は必要。
- いままで意味のないコメントしか見たことない: 書き方を勉強すればいい

コメントの目的は、設計者の意図でコードに表せないものを書くこととのこと。それによってChapter 2.　で述べられていた認知負荷とunknown unknownを抑える。

# Chapter 13. Comments Should Describe Things that Aren’t Obvious from the Code

コードからは非自明なことはコメントで説明されるべきだと述べられている。プログラミング言語は全ての意図を表現するようにはなっていないので、足りない部分（つまりコードからは自明でないこと）はコメントで説明されるべき。

自明であることと、コードを頑張って読めばわかることは異なっている。コードは何かしらの抽象化されたインターフェースを介して機能を提供しているはずで、モジュールはその抽象化のレベルで理解できるようになっていて欲しい。
インターフェースを定義するためにコードを書くこともあるし、それをモジュールのクライアントを実装するユーザが読むこともあるだろう。ただ、そのコードだけではインターフェースを理解できないならば、コメントで抽象化の説明を補うべきである。

コメントを書くためにフォーマットを選んでおくと良い。言語ごとに標準の仕組みがあるし、言語ではなくフレームワークにも仕組みがある。
一貫性が生まれるし、そもそも何を書けばいいかが定まるから書くのが簡単になる。何を書くか決めるのは大変なので、毎回やってられない。

コメントには何種類かある。インターフェースの説明とデータ構造の説明が大切。

- インターフェース: 抽象化の説明をする。モジュールのインターフェースを説明する
- データ構造: データ構造の各フィールドの説明をする
- 実装: コードの動作の説明を補足する
- モジュール間の依存

実装のことは説明しても外部に見えないものなので、重要でないのはそう。モジュール間の依存はそもそも説明しないといけないような事態にならないようにすべきだろう。その説明が必要ならば、一個のモジュールにまとめるのが良いはず。

コードが表現する抽象化レベルがある。それと同じレベルでコメントを書いても大体同じ意味の繰り返しになるので書かないほうがいい。
より低いレベル（詳細な内容）のコメントを書くことで、解釈を補足できる。単位とかnullの解釈、不変条件などがそれ。コードを読めばわかることは多いだろうけど、自明じゃないしそれをインターフェースに含める意図があるかが明らかではない。
より高いレベルのコメントは、むずい実装の背景とか目的を説明する。理解を簡単にする。あまりインターフェースの説明って感じではなさそう？

あとは箇条書きで記憶を頼りに感想をメモ。ここまでと以下の箇条書きを書いたタイミングが異なるので、矛盾や重複があるかもしれない。

- インターフェースのコメントと実装のコメントは役割が違うので区別した方がいい
  - 他にもいくつかあった気がするけどまあいいでしょう
- インターフェースのコメントはそのモジュールが提供する抽象化を説明するもの
  - 標準ライブラリのドキュメントはこれ
  - 読み手にとって明らかじゃないことは全て説明されるべきで、それがコード（関数や引数の名前とか型とか）でもいいし、コメントでもいい
  - コードで表現できることには限りがあるので、コメントで説明するのが便利
- 実装のコメントは実装の理解を簡単にするためのもの
  - そういう実装になっている背景とか、どういう不変条件を保とうとしているかとかを述べる
- 読者のことを思いやろうということと、コメントにはそれぞれ異なる読者がいることを指摘しているように思った
  - インターフェースの読者はユーザ
  - 実装の読者はメンテナ
- 読者のことを考える、でこの本を思い出しました
  - https://www.hyuki.com/mw/
  - 数学ガールの著者が書いたやつです、初めて読んだ文章の書き方本でよかったです
...
一貫性を保つと認知負荷が減って複雑さが抑えられる。

一貫性を発揮できる場所の例として名前、コーディングスタイル、インターフェース（いろんな実装が一つのインターフェースを満たす）、デザインパターン、不変条件が挙げられていた。
不変条件は認知負荷を下げるためのツールというより、抽象化を提供するためのツールだと思っていてここに並ぶのをみたときに少しギョッとしたのだが、思えばどれも抽象化を提供するためのものではあった。

今の時代は生成AIがあるのでドキュメントがあればlinterの他にAIに頼めるから便利。

# Chapter 18. Code Should be Obvious

コードの明らかさはコードレビューで測れると冒頭で述べていた。

これまでは開発チーム内でのコードレビューでそれを担保していたが、それでは開発チームのメンバーにとって明らかであることしか担保できない。
今の時代はAIエージェントにとって明らかであることを担保する必要があるはず。

AIにレビューさせればAIエージェントにとって理解しやすいコードであることを担保できるかというと怪しいかと思う。どうするのがいいかなあ

<details>

<summary> Gemini に聞いてみた </summary>

AIエージェント（Claude Code, Cursor, Devinなど）にとって「理解しやすい（Obviousな）」コードベースを構築するというテーマは、現在まさに急速に発展している領域です。

従来の「人間にとっての可読性」と共通する部分が多い一方、AI特有の**「コンテキスト窓の制限」**や**「暗黙のルールの欠如」**を補うための新しい設計論が議論され始めています。

参考になる書籍やリソース、および重要な概念を整理しました。

---

## 1. 注目すべき書籍・文献

### 『Agentic Coding Pro: The Structured Path to Autonomous Software』 (2025)

AIエージェントを単なる「アシスタント」ではなく、自律的な「共同作業者」として扱うためのアーキテクチャについて詳述している最新の書籍です。

* **主な内容:** エージェントが迷わないための「PLANフレームワーク（Prepare, Launch, Audit, Nurture）」や、コードベースのメンテナンスをAIに任せるための構造化手法について解説されています。

### 『Creating AI-Friendly Codebases』 (Davide Consonni, 2025)

Medium等で公開されているナレッジの集大成で、AIにとっての「認知負荷」をどう下げるかに焦点を当てています。

* **キーコンセプト:** 「暗黙的な振る舞い（Magic）」を避け、型定義や明示的なインターフェースを重視する設計（Explicitness over Magic）を推奨しています。

### 論文: 『Agent READMEs: An Empirical Study of Context Files for Agentic Coding』 (arXiv, 2025)

エージェントのためのREADME（`CLAUDE.md` や `AI.md`）が、エージェントのパフォーマンスにどう影響するかを実証的に調査した論文です。

* **要点:** プロジェクトの全体構造、特定の設計判断、避けるべきパターンを「エージェント専用のドキュメント」として明文化することの有効性を説いています。

---

## 2. AIにとって「Obvious」なコードにするための3大原則

AIエージェントの挙動を安定させるために、以下の3つのレイヤーでコードを整理することが推奨されています。

### ① インターフェースの明示化（Explicitness）

AIは「空気を読む」ことが苦手です。メタプログラミングや、実行時に動的に型が決まるような「マジック」は、エージェントの推論を狂わせます。

* **型ヒントの徹底:** TypeScriptやPythonのType Hintsなど、静的解析が可能な情報を最大限付与する。
* **副作用の分離:** 関数が何を変更するのかをドキュメントではなく、シグネチャ（引数と戻り値）から明確に判断できるようにする。

### ② コンテキスト・ガイドの設置

エージェントがファイルを開いた瞬間、そのプロジェクトの「ルール」を理解できるようにします。

* **`CLAUDE.md` / `AI.md` の運用:**
* ビルド・テストのコマンド。
* 「このプロジェクトではDRYよりも可読性を優先する」といった方針。
* 特有のアーキテクチャ（例：Clean Architecture、DDD）の採用有無。


* **ディレクトリ構造の標準化:** 予測可能な構造（`src/components`, `src/services` など）を採用し、エージェントが「次にどのファイルを見るべきか」を推測しやすくする。

### ③ 検証可能性（Verifiability）

AIが変更を加えた後、それが正しいかどうかを「自律的に」確認できるようにします。

* **高速なテストスイート:** エージェントが数秒で実行できるユニットテストがあることは、エージェントの修正成功率を劇的に高めます。
* **自己記述的なエラーメッセージ:** 失敗時に「何が」「なぜ」ダメだったのかをエージェントに伝えるエラー設計。

---

## 3. 次のステップとしてのおすすめ

もし、今すぐ自分のコードベースをAIフレンドリーにしたいのであれば、Anthropicが提唱している **「CLAUDE.md」の導入** から始めるのが最も実用的です。

* **CLAUDE.md に書くべき内容:**
1. **Tech Stack:** 使用している言語、フレームワーク、ライブラリのバージョン。
2. **Coding Guidelines:** 改行のルールから、非推奨の関数リストまで。
3. **Architecture:** モジュール間の依存関係のルール。



この手法は、人間にとっても「オンボーディング資料」として極めて優秀に機能します。

**まずは、ご自身のプロジェクトに `CLAUDE.md`（または `AGENTS.md`）を作成し、AIエージェントに「このプロジェクトの憲法を書いて」と依頼してみるのはいかがでしょうか？**
</details>
